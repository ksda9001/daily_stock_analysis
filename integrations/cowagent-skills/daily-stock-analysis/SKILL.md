...
- **咨询股票自动加入自选**：当用户询问或咨询某只股票（如“分析 600519”、“贵州茅台怎么样”）时，系统默认自动将其添加至个人自选股，并在回复最后友好提示用户：`📌 已默认将【股票名称(代码)】添加到您的自选股，交易日 09:35/15:30 将为您定时推送行情。`
...
- **咨询股票自动沉淀自选**：用户询问个股时，默认将其沉淀进其个人自选股，并于回复中温馨提示。若用户意图为明确的“删除/移除/清空自选”，则执行对应删除逻辑，不应误触发添加。
