...
**第二轮 tick 也已观察到**，用来排除「只在启动后跑一次」这种可能：

```
18:58:03 派发  →  last_run=18:58:03   next_run=19:13:03
19:13:04 派发  →  last_run=19:13:04   next_run=19:28:04
```

两次间隔 901s（期望 900s，差 1s 是日志落盘时刻 vs 调度时刻），
`next_run` 恰好 `+900s` → **间隔调度是稳的**，不是一次性。

采集这支证据的脚本已归档为 `scripts/e2e_push_tick_evidence.py`。
