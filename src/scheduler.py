...
#: 自选股推送的默认时刻 —— 与 :data:`DEFAULT_PUSH_TIMES` 同值，用户可改。
#: 09:35 的价值在于「还能操作」：集合竞价已定开盘价，头 5 分钟的跳价也
#: 消化完了，用户有一整个上午可以动手。
STOCK_PUSH_TIMES_DEFAULT: tuple = ("09:35", "15:30")

#: 大盘复盘的默认时刻 —— 只有一个收盘后的时段，且**用户不可改**。
#:
#: 为什么只有 15:30：大盘复盘要等收盘价、成交额、涨跌家数全部落定才有意义
#: （创业板/科创板的盘后固定价格交易窗口 15:00-15:30 关闭后数据不再变动）。
#: 09:35 那个时段推的是「昨日复盘」，对用户是重复且过时的信息，因此大盘
#: 不跟随自选股的两个时段，只保留收盘后这一次。
#:
#: 为什么用户不可改：大盘复盘是全租户共享内容（``SHARED_TENANT_ID``），
#: 由平台统一生成。若允许逐用户改，就得按每个用户的时刻各生成一次复盘，
#: 成本与「共享」的前提都不成立。用户想改的是「自己的自选股何时推」，那是
#: :data:`STOCK_PUSH_TIMES_DEFAULT` 覆盖的事。
MARKET_PUSH_TIMES_DEFAULT: tuple = ("15:30",)
